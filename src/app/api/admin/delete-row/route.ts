...
export async function POST(request: NextRequest) {
  try {
    ...
    const { serverId, table, id } = body;

    if (!serverId || !table || id === undefined) {
      return NextResponse.json({ error: 'Server ID, table, and ID are required' }, { status: 400 });
    }
    ...
    try {
      ...
      // Sanitize table name and execute delete
      ...
      console.error('Delete row error:', dbError);
    ...
    console.error('Delete admin row error:', error);
    return NextResponse.json({ error: 'Failed to delete row' }, { status: 500 });
  }
}